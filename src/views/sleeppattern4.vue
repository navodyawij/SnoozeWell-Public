...
export default {
  ...
  setup() {
    ...
    const isFormValid = computed(() => {
      return sleepEnvironment.value !== null && 
             consumesBeforeBed.value !== null;
    });
    ...
    const saveToFirestore = async () => {
      try {
        ...
        const environmentData = {
          sleepEnvironment: environmentTexts[sleepEnvironment.value],
          consumesBeforeBed: consumesBeforeBed.value ? 'Yes (Caffeine, Alcohol, Both)' : 'No',
        ...
        router.push("/auth/sleeppattern5");
    ...
    const goToNext = () => {
      // Basic validation
      if (!sleepEnvironment.value) {
        alert("Please select your sleep environment.");
        return;
      }
      
      if (consumesBeforeBed.value === null) {
        alert("Please indicate whether you consume caffeine or alcohol before bed.");
        return;
      }
